...
test_file = location + 'rsc15_test_ole.csv'
...
with tf.Session() as sess:
    ...
    for e in range(max_epochs):
        ...
        num_batches = int(n_training_examples/batch_size)
        print("Num batches:", str(num_batches), "| progress: ", end='')
        progres_part = int(num_batches/10)
        next_land_mark = progres_part
        lm_t = time.time()
        ...
        for b in range(num_batches):
            ...
            epoch_loss += l
            if b==next_land_mark:
                print(str(b), str(time.time()-lm_t), end=' ')
                lm_t = time.time()
                next_land_mark += progres_part
